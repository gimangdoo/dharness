...
**검증 7단계:**
1. **구조 검증** — 파일 위치, frontmatter(name/description), 참조 일관성, `.claude/commands/` 미생성 확인
...
Phase 0에서 "운영/유지보수"로 분기 시 진입.

1. **현황 감사** — `.claude/agents/` ↔ 오케스트레이터 에이전트 구성 비교, `.claude/skills/` ↔ 스킬 구성 비교 → 불일치 보고
...
| 트리거 | 조건 |
|------|------|
| **수동** | "하네스 점검", "drift 확인", "적응", "baseline 갱신" 등 키워드 |
