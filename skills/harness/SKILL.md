...
명시 요청("하네스 수정해줘", `/harness-evolve <피드백>`)뿐만 아니라:
- 같은 유형의 피드백이 2회 이상 반복 → `/harness-audit`로 구조적 점검 권고
- 에이전트가 반복 실패하는 패턴 → `/harness-adapt`로 telemetry 기반 사용 drift 분석 권고
- 사용자가 오케스트레이터 우회하여 수동 작업 → `/harness-audit`의 변경 이력 누락 검사 권고

#### 9-5. 운영/유지보수 워크플로우

Phase 0에서 "운영/유지보수"로 분기 시 진입. 명시적 진입점은 `/harness-audit` (read-only 정합성 감사).

1. **현황 감사** — `.claude/agents/` ↔ 오케스트레이터 에이전트 구성 비교, `.claude/skills/` ↔ 스킬 구성 비교 → 불일치 보고 (`/harness-audit`이 자동 수행)
...
| 트리거 | 조건 |
|------|------|
| **수동** | "하네스 점검", "drift 확인", "적응", "baseline 갱신" 등 키워드 / `/harness-adapt` 명시 호출 |
...
> 상세 telemetry schema, capture 신호 목록(7종 이벤트), diagnostic 룰, adapt 룰, 승인 UX는 `references/runtime-adaptation.md`.
