...
| 사용자 명시 요청 ("되돌려줘", "방금 변경 취소", `/harness-adapt --rollback {ts}`) | 해당 ts 스냅샷으로 복원 |
...
| 수동 | 사용자 요청 ("CLAUDE.md 변경 이력 정리", `/harness-audit --compact-changelog`) |
...
| 스킬 description 변경 | 8-4 트리거 검증 | should-trigger 8개 + should-NOT 8개 자동 재실행, 트리거율 회귀 시 fail |
...
- description 변경의 트리거 검증은 LLM 호출 16회(should-trigger 8 + should-NOT 8)로 비용이 큼. 변경 묶음에 description 변경이 여러 건이면 한 번에 배치 실행하여 컨텍스트 재사용.
