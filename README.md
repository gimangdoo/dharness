...
**검증:** `claude mcp list` → `✓ Connected` 확인. 실패 시 stderr 첨부.

> ⚠️ **경로 인자는 절대경로 필수** — `--db-path ./...`, `--repository ./...` 등 상대경로는 health check cwd 차이로 `✗ Failed to connect` 발생. uvx 실행자 자체도 PATH 미통과 시 spawn 실패하므로 절대경로 (`%APPDATA%\Python\Python312\Scripts\uvx.exe`) 권장.
...
### dharness 권장 패턴 — inline `mcpServers:` (서브에이전트 격리)
...
> **schema 필수 (9차 사이클 확정):** **list-of-dicts + `type: stdio`** 형태. 누락 시 silent skip 되어 도구 미노출. 자세한 합성 템플릿은 [§5-1 inline `mcpServers:`](./plugins/harness/skills/harness/references/permission-profiles.md#5-1-에이전트-frontmatter-layer-b-격리--권장-inline-mcpservers).
...
→ `COUNT=N` + per-tool `name·required·all-params` 출력. install 부작용 0 (단 패키지 코드는 1회 spawn). 자세한 절차는 [§8-3 재사용 가능 검증 기법](./plugins/harness/skills/harness/references/permission-profiles.md#8-3-재사용-가능한-검증-기법).
...
`claude mcp add` 또는 inline `mcpServers:` 갱신은 **본 세션에 미적재** — 새 도구가 LLM에 노출되려면 **세션 재시작** 필수 (4차 사이클 양면 empirical). 합성 시점 install된 신규 MCP도 동일.
...
| 검증 완료 T0 MCP × capability profile 매트릭스 | [`permission-profiles.md` §3-1](./plugins/harness/skills/harness/references/permission-profiles.md#3-1-검증-완료-t0-mcp--capability-profile-매트릭스-14차-사이클-p2-1차-종합-보고) |
...
| Pre-install probe fixture 4종 | [`fixtures/probe_*.js`](./plugins/harness/skills/harness/references/fixtures/) |
...
| [`plugins/harness/skills/harness/references/permission-profiles.md`](./plugins/harness/skills/harness/references/permission-profiles.md) | MCP·도구 권한 카탈로그 (Tier 분류 / §3 인벤토리 / §10 dynamic adoption / §11 검증 fixture) |
