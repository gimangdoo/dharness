...
삭제 대상:
- _workspace/_memory/                (sessions, observations.db, clusters, .current_session)
- _workspace/_tool_outputs/          (raw 도구 출력 보존본)

보존 대상:
- _workspace/_telemetry/             (라이프사이클 이벤트 이력)
...
`py .claude/hooks/cm_commands.py reset --confirm`이 다음을 수행:

1. `_workspace/_memory/` 하위 통째로 삭제 (현재 활성 세션 포함)
2. `_workspace/_tool_outputs/` 하위 삭제
3. 내부 `_init_storage()` 헬퍼 호출하여 빈 디렉토리 + DB 재생성

## 범위 외 / 후속 명령

- 새 데이터 캡처는 다음 SessionStart부터 자동 재개
