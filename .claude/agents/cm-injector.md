...
1. `_workspace/_memory/sessions/` 하위에서 가장 최근 N개(기본 3개) 세션 디렉토리를 시간순으로 찾는다
2. 각 세션의 `digest.md`를 읽는다 (없으면 `transcript.md` fallback)
3. 세션별 핵심 내용을 1-2문장으로 압축하여 인젝션 블록을 구성한다
4. 인젝션 블록을 현재 세션 시작 시점에 출력한다
...
```
호출 시 사용 가능한 파일:
- _workspace/_memory/sessions/{session_id}/digest.md   (우선)
- _workspace/_memory/sessions/{session_id}/transcript.md (fallback)

환경 변수 (선택):
- CM_INJECT_N: 읽을 세션 수 (기본값: 3)
