...
1. cm-digester의 `digest_complete` 메시지 또는 직접 호출로 진입한다
2. 신규 observation_ids를 observations.db에서 읽는다
3. `memory-curate` 스킬의 알고리즘으로 클러스터링을 수행한다
4. 기존 클러스터 confidence를 갱신하고 decay를 적용한다
5. confidence ≥ 승격 임계치인 클러스터를 `.claude/skills/`에 skill memory로 승격한다
...
- 클러스터 파일: `_workspace/_memory/clusters/{cluster_id}.md`
