...
description: |
  ...
  "최근 schema 변경 이유는?", "지난번에 worker를 어떻게 단순화했지?", "phase 5 산출물
...
| 3 | `CLAUDE.md` "변경 이력" 표 | 사람이 큐레이션한 narrative + 사유 (가장 신호 좋은 source) | Read + grep |
| 4 | `git log` / `git show` | commit message + diff (코드 단위 변경 history) | Bash |
| 5 | `clusters` / `daily_summaries` / skill 본문 | 반복 패턴 + 일자 요약 + 도메인 정의 | SQL + Read |
...
category 카탈로그: `cm_hook_edit`, `cm_schema_edit`, `cm_command_edit`, `cm_skill_edit`, `cm_settings_edit`, `cm_worker_edit`, `cm_doc_edit`, `cm_deps_edit`, `harness_skill_edit`, `harness_command_edit`, `harness_reference_edit`, `harness_manifest_edit`, `claudemd_edit`, `marketplace_edit`, `readme_edit`, `gitignore_edit`, `git_commit`, `git_add`, `git_rm`, `git_mv`, `git_push`, ...

artifact_kind 카탈로그: `skill`, `agent`, `command`, `hook`, `schema`, `claude_md`, `plugin_manifest`, `doc`, `deps`, `settings`, `worker`, `worker_static`, `reference`, `git`, `harness`, `config`.
...
#### 2c. 클러스터/요약 단위 — `clusters` + `daily_summaries`
...
- 산출물: .claude/{hooks,commands,skills}, worker/, marketplace.json 단일화
