...
2. **Context Manager** (root `.claude/` + `_workspace/`) — dharness 본 폴더 한정 self-host 런타임. 결정적 hooks 3종 + `/cm-*` 5 슬래시 커맨드 + `memory-search` 1 스킬. PostToolUse가 dharness 작업 단위(skill/agent/hook/command/manifest 변경)를 자동 분류해 누적하고, SessionEnd가 `CHANGELOG.md` "변경 이력" 표 행 draft를 자동 적재. **dharness 본 폴더 영구 부속** — 외부 install 미지원.
...
변경 이력 정본은 [`CHANGELOG.md`](./CHANGELOG.md). CLAUDE.md는 매 세션 컨텍스트에 로딩되므로 자주 바뀌는 이력은 분리한다 (Claude Code CLAUDE.md litmus — `code.claude.com/docs` memory.md). CM SessionEnd가 draft를 `_workspace/_drafts/`에 적재하고 `/cm-claudemd-apply <sid> [사유...]`가 `CHANGELOG.md` 표에 삽입한다.
