...
| **P5** | #5 wiki 다리 **실행 자동화** | ⬜ 미착수 | P4가 계약/스키마/게이트만 박제 → 실 송수신 I/O는 산문 지시뿐(코드 0). P5 = 산문→재사용 헬퍼 `scripts/wiki/bridge.py`(emit/pull/dedup) + skip telemetry 3 이벤트 강제 + e2e dogfood 1회. **self-host 전용**(derived 런타임 주입 0, 사용자 결정 2026-06-03). dedup 임계=wiki rule 2-a 상속(fork 금지)·advisory soft-block. wiki repo 실쓰기는 사용자 gate 유지 |
