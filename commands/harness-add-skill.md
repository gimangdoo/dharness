...
| 항목 | 안내 |
|---|---|
| (1) | "에이전트가 없습니다. `/harness-new` 또는 `/harness-add-agent`를 먼저 실행하세요." |
| (2) | 사용자에게 "어느 에이전트가 이 스킬을 사용할 예정인가요? (1개 또는 N개)" 질문 |

## 실행 절차

`skills/harness/SKILL.md`의 워크플로우를 따르되 **Phase 1·2·3·4·5는 SKIP**한다.
...
1. **Phase 6 (스킬 생성)**:
   ...
   - 작성 룰 전체는 `skills/harness/references/skill-writing-guide.md`
...
- 새 에이전트 생성 → `/harness-add-agent`
- baseline 갱신 → `/harness-baseline`
- 기존 스킬 description만 수정하고 싶으면 → 직접 편집 + `/harness-audit`로 정합성 확인
