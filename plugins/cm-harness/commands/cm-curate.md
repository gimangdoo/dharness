...
본 커맨드는 LLM 작업이므로 결정적 핸들러(`plugins/cm-harness/hooks/cm_commands.py`)에서 처리하지 **않는다**. Claude는 다음 절차로 cm-curator 에이전트를 직접 호출한다:
...
`/cm-harness:cm-curate`는 명시적 호출. 자동 호출은 cm-orchestrator가 다음 조건에서 동일한 에이전트를 호출한다:
...
- 승격 후 회귀 검증 — `/harness:harness-adapt`의 Phase 8 회귀 검증 활용
