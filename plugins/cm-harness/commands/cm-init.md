...
> **`_baseline/cm_baseline.json`은 본 커맨드 범위 외:** 파일은 git에 체크인된 템플릿 형태로 이미 존재하며, 초기 30 세션 누적 후 cm-curator가 `initial_avg_*` 필드를 자동으로 채운다. 누락 시 수동 복원 또는 `/harness-baseline` 영역.
