...
> **휴리스틱 임계값 baseline:** Phase 1 분석 임계값은 [`heuristics-baseline.md §1`](heuristics-baseline.md#1-phase-1--code-research) 단일 출처. 도메인별 override는 `_baseline/project_profile.md`의 `analysis_overrides` 필드 박제.
...
8. [Maturity / Pain Points 축 조사 (deep only)](#8-maturity--pain-points-축-조사-deep-only) — 상세 [code-research-deep.md](code-research-deep.md)
9. [신뢰도 결정 알고리즘](#9-신뢰도-결정-알고리즘)
10. [병렬 실행 전략](#10-병렬-실행-전략)
11. [폴백 전략](#11-폴백-전략)
...
1. **측정값 + 근거** — 모든 추출 값은 `source` 필드와 함께 기록. 단순한 형식이 아니라, 사용자 검증·confidence 결정·Phase 10 drift 추적의 기반.
...
## 8. Maturity / Pain Points 축 조사 (deep only)

> deep audit 전용 2축(Maturity·Pain Points) 상세 조사 기법은 [code-research-deep.md](./code-research-deep.md) 단일 출처로 분리 (2026-05-31 doc 분할 #2). quick scan은 §5-7만, deep audit 시 본 2축 추가 수집.
