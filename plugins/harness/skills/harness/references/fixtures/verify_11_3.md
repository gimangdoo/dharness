...
| 2026-05-11 | dharness-probe-test 인터랙티브 `claude` (P3-(c) 21차 B8 — gate ALL ON, 13차 B1과 동일 조건) | sqlite (`.mcp.json` type:stdio + abs uvx + abs db) + gate 4 모두 ON (`<derived>/.claude/settings.json={"enabledMcpjsonServers":["sqlite"]}` + `~/.claude.json` 4 키 모두 충족) | **M2=6** (sqlite 6종 모두 적재: `mcp__sqlite__{append_insight, create_table, describe_table, list_tables, read_query, write_query}` §3 인벤토리 100% 일치) | (B9 행에서 측정) | (B9 후 판정) | **인터랙티브 모드 baseline 측정 ✓** — `-p` 모드 13차 B1과 동일 결과. 외부 PowerShell 1탭으로 사용자 측정 회수. |
| 2026-05-11 | dharness-probe-test 인터랙티브 `claude` (P3-(c) 21차 B9 — `<derived>/.claude/settings.json` `enabledMcpjsonServers=[]` 토글 OFF) | sqlite (`.mcp.json` 미변경) + `<derived>/.claude/settings.json={"enabledMcpjsonServers":[]}` (settings.json만 OFF, `~/.claude.json` 4 키 ALL ON) | (B8 행 참조) | **M2=6 — B8과 동일** (sqlite 6종 모두 그대로 적재 — `append_insight`/`create_table`/`describe_table`/`list_tables`/`read_query`/`write_query`). 인터랙티브 모드에서도 settings.json 토글 deferred pool 영향 0 | M2=6 | **✗ Layer A 없음 — `<derived>/.claude/settings.json` `enabledMcpjsonServers` 토글은 모드 무관 deferred pool 영향 0** | 🚨🚨🚨🚨🚨 **5차 결정적 empirical (P3-(c) closure — mode-independent falsification 확정)**: 인터랙티브 `claude`에서도 B8=B9=6 — fixture의 "가장 가능성 높은 결과"(mode-independent falsification) 적중. **13·15·16차 `-p` 모드 한정 caveat *제거*** — 결론 mode-independent로 일반화: project-scope `<derived>/.claude/settings.json` `enabledMcpjsonServers` 토글은 deferred pool 적재를 *전혀* 제어하지 못함, *인터랙티브·`-p` 모드 무관*. `claude -p` auto-trust 부수 효과 가설 ✗ 부정. **§1 [^A] 진짜 절감 채널 후보가 *server-side `--toolsets`/env*로 유일하게 좁혀짐** — 그것이 미측정 + 유효 가설로 남는 유일 채널. **P3 결정적 미해소 분기 완전 closure** (T1 probe / T1+ API 키는 외부 환경 + 영구 미완 의제로 분리). |
| 2026-05-11 | dharness-probe-test + P3-(c) 21차 FINAL restore (사용자 외부 PowerShell) | `<derived>/.claude/settings.json={"enabledMcpjsonServers": ["sqlite"]}` (baseline 복원) | (측정 없음) | (측정 없음) | restore ✓ | **P3-(c) 21차 사이클 closure** — 측정 종료 후 baseline 상태 사용자 복원. P3 영역의 결정적 미해소 분기 모두 종료. 남은 의제는 T1 probe (외부 환경) + T1+ API 키 (영구 미완) — 모두 dharness root 20차 closure 의제와 정합. |
