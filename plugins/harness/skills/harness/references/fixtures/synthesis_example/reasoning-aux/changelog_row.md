...
본 행은 derived 프로젝트의 변경 이력에 **추가** — 기존 행을 수정하지 않음. plugin host `CHANGELOG.md`의 "변경 이력" 표(host self-host CM 운영 시)와 동일 규약.
...
3. 본 `changelog.md` "변경 이력" 표 행은 *유지* (rollback 사실을 다음 행으로 추가)

plugin host `CHANGELOG.md` 변경 이력 표의 append-only 규약과 동일.
