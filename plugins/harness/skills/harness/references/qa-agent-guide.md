...
**권장**: QA 에이전트는 검증·수정 양쪽 권한을 갖는 일반 에이전트 타입으로 설정. 단 "검증 → 리포트 → 수정 요청" 프로토콜을 정의에 명시해 *발견*과 *수정*을 분리. 도구 권한은 `permission-profiles-profiles.md §2` capability profile 매칭 결과를 따른다.
