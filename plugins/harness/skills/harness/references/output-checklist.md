...
- [ ] **Runtime gate 블록** — 오케스트레이터 `### Phase 1: 준비` 직후 `<!-- RUNTIME-GATE:start -->`…`<!-- RUNTIME-GATE:end -->` 1쌍 박제 (런타임 규모분류/plan분해/산출물생성 스캐폴드, 단일 출처 `references/runtime-execution.md`, `chain.py:check_runtime_gate_block` 결정적 검증). P0 단계는 G1/G2/G3 골격 표만 — silent 생략 금지
