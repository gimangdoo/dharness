...
4. **Phase 10 telemetry 기록 (필수, 누락 금지)** — 자세한 형식·이벤트 카탈로그는 [§Phase 10 Telemetry 강제 블록](#phase-10-telemetry-강제-블록) 참조. 워크플로우 종료 직전 무조건 실행.
5. 사용자에게 결과 요약 보고
...
### Phase 4: 정리
1. `_workspace/` 보존
2. **Phase 10 telemetry 기록 (필수, 누락 금지)** — 자세한 형식은 [§Phase 10 Telemetry 강제 블록](#phase-10-telemetry-강제-블록) 참조. 워크플로우 종료 직전 무조건 실행.
3. 결과 요약 보고
...
### Phase 5: 정리 + telemetry (필수)
1. `_workspace/` 보존
2. **Phase 10 telemetry 기록 (필수, 누락 금지)** — 자세한 형식은 [§Phase 10 Telemetry 강제 블록](#phase-10-telemetry-강제-블록) 참조
3. 결과 요약 보고
```
...
8. **검증 게이트 토폴로지를 의식적으로 선택** — 전담 순수 reviewer(`tools: [Read, Grep]`)냐 최종 에이전트 겸직이냐는 `agent-design-patterns.md` "검증 게이트 토폴로지" 기준으로 결정하고, 겸직이면 사유를 본 SKILL.md에 한 줄 박제. 한 하네스 안에서 섞지 않는다
...
## CLAUDE.md HOW 본문 draft (Q3 doctrine, 2026-05-16)

> **Read at phase:** Phase 7-4.5 (HOW draft 생성) — 코드 도메인 한정 권장, 비코드 도메인 생략.
...
- 7-4 템플릿의 운영·구조·규칙은 *항상* 박제. 본 HOW(스택)는 코드 프로젝트에서 권장(= `/init` 동급 콘텐츠), 비코드 도메인은 생략.
...
`_workspace/_telemetry/{YYYY-MM-DD}.jsonl`에 *한 줄 한 객체* 형식으로 append:

| 이벤트 | 발생 시점 | 페이로드 (필수 필드) |
|--------|---------|-------------------|
| `harness_invocation` | 오케스트레이터 시작 1회 | `{ts, type, session_id, trigger_keyword}` |
| `agent_invocation` | 각 에이전트 호출 1회 | `{ts, type, session_id, agent, duration_s, outcome}` |
| `agent_failure` | 에이전트 실패/타임아웃마다 | `{ts, type, session_id, agent, reason}` |
| `tool_output_captured` | 도구 출력 10KB 초과 시 (PostToolUse hook이 자동 emit — derived에서는 *수동 emit 불필요*) | — |

### 페이로드 예시

```jsonl
{"ts":"2026-05-12T10:00:00Z","type":"harness_invocation","session_id":"abc123","trigger_keyword":"launch"}
{"ts":"2026-05-12T10:00:05Z","type":"agent_invocation","session_id":"abc123","agent":"researcher","duration_s":120,"outcome":"success"}
{"ts":"2026-05-12T10:02:05Z","type":"agent_invocation","session_id":"abc123","agent":"editor","duration_s":45,"outcome":"failure"}
{"ts":"2026-05-12T10:02:10Z","type":"agent_failure","session_id":"abc123","agent":"editor","reason":"timeout"}
```

`outcome`은 `success` / `partial` / `failure` 중 하나. `partial`은 일부 결과 산출 후 중단 — `agent_failure` 추가 emit 권장.
...
date_d=$(date -u +"%Y-%m-%d")
mkdir -p _workspace/_telemetry
printf '{"ts":"%s","type":"harness_invocation","session_id":"%s","trigger_keyword":"%s"}\n' \
  "$ts" "{id}" "{kw}" >> "_workspace/_telemetry/$date_d.jsonl"
```

### 누락 방지 체크리스트

워크플로우 마지막 phase가 다음을 *반드시* 보장한다:

1. `_workspace/_telemetry/{date}.jsonl` 파일이 *생성*됐는지 (`Test-Path` / `test -f`)
2. `harness_invocation` 1줄 + N개 에이전트별 `agent_invocation` 1줄씩 적재됐는지
3. 실패 에이전트가 있었다면 `agent_failure` 추가 1줄씩
