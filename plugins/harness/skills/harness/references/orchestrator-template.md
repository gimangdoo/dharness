...
4. **Phase 10 telemetry 기록** — `_workspace/_telemetry/{YYYY-MM-DD}.jsonl`에 이 세션의 이벤트를 append:
   ```jsonl
   {"ts":"{ISO8601}","type":"harness_invocation","session_id":"{id}","trigger_keyword":"{키워드}"}
   {"ts":"{ISO8601}","type":"agent_invocation","session_id":"{id}","agent":"{name}","duration_s":{n},"outcome":"{success|partial|failure}"}
   ```
   에이전트별로 `agent_invocation` 1줄씩 추가. 실패 시 `agent_failure` 이벤트 추가.
5. 사용자에게 결과 요약 보고
...
### Phase 4: 정리
1. `_workspace/` 보존
2. **Phase 10 telemetry 기록** — `_workspace/_telemetry/{YYYY-MM-DD}.jsonl`에 append:
   ```jsonl
   {"ts":"{ISO8601}","type":"harness_invocation","session_id":"{id}","trigger_keyword":"{키워드}"}
   {"ts":"{ISO8601}","type":"agent_invocation","session_id":"{id}","agent":"{name}","duration_s":{n},"outcome":"{success|partial|failure}"}
   ```
