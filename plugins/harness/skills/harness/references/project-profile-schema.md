...
| baseline 재실행 활용 | 의도 변화 신호 | 코드 변화 신호 |

### 핵심 설계 원칙

1. **신호 + 근거 분리** — 모든 추출 값에 "어떤 파일/명령에서 나왔는지" 근거를 함께 기록한다. 단순히 `language: typescript`가 아니라 `language: typescript (source: package.json#devDependencies, tsconfig.json 존재)`. 근거는 (a) 사용자 검토 시 신뢰성 판단, (b) Intent Profile 자동 채우기 시 confidence 결정, (c) baseline 재실행 시 변화 추적 anchor.
...
4. **비교 기준점** — 이 schema의 측정값들이 baseline 재실행 시 비교 anchor — 변경되면 후속 Phase 영향 분석의 신호.
...
이 규약은 baseline 재실행 시 "어떤 파일을 다시 봐야 하는지" 결정하는 데 사용된다.
