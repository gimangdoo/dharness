...
| `wiki_bridge_skipped` | `session_id`, `milestone`, `reason` | wiki 다리 skip 시 — `milestone`=`emit`/`pull`, `reason`=`target-absent`/`not-reusable`/`dedup-hit`/`rollback-failed`. `not-reusable`은 LLM 비-Emit 결정 시 `bridge.py:note_skip` 호출로 박제 |
