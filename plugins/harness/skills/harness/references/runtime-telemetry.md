...
| `wiki_bridge_emit` | `session_id`, `slug`, `version`, `path`, `outcome` (실패 시 `n_errors`+`errors`) | M1 Emit 직후 (`scripts/wiki/bridge.py:emit_candidate`). `path`=candidate 디렉토리명(slug-vN) — 디스크 상관 |
| `wiki_bridge_skipped` | `session_id`, `milestone`, `reason` | wiki 다리 skip 시 — `milestone`=`emit`/`pull`, `reason`=`target-absent`/`not-reusable`/`dedup-hit`/`rollback-failed`/`unreadable-candidate`. `not-reusable`은 LLM 비-Emit 결정 시 `bridge.py:note_skip` 호출로 박제 |
| `wiki_bridge_pull` | `session_id`, `n_promoted`, `n_scanned`, `n_skipped_unreadable`, `slugs` | M4 Feedback 조회 직후 (`scripts/wiki/bridge.py:pull_promoted`). scan denominator + 무음 드롭 수 노출 |
