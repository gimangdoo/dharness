...
> **휴리스틱 임계값 baseline:** Phase 10 임계값은 [`heuristics-baseline.md §3`](heuristics-baseline.md#3-phase-10--runtime-adaptation) 단일 출처. 도메인별 override는 `_baseline/intent_profile.md`의 `tuning_overrides` 필드 박제.
...
3. [Telemetry Schema](#3-telemetry-schema) — 상세 [runtime-telemetry.md](runtime-telemetry.md)
...
> telemetry 저장 형식(`{date}.jsonl`)·이벤트 타입 표(7종)·공통 규약은 [runtime-telemetry.md](./runtime-telemetry.md) 단일 출처로 분리 (2026-05-31 doc 분할 #3). 본 문서 §4 이하 Capture/Diagnostic/Adapt 레이어가 이 schema를 소비.
...
> **도메인 적용 범위:** 아래 신호 표는 *코드 프로젝트* 기본값이다. 비코드 도메인(재무·교육·생활·문서 등)은 의존성·커버리지·LOC가 무의미하므로, 도메인 신호(입력 데이터 스키마 변경, 산출물 카테고리 추가 등)로 치환하거나 baseline drift 캡처를 future scope로 명시한다 (`SKILL.md` Phase 10-7). 사용 신호(아래 "Usage Signals")는 도메인 무관 — 항상 캡처.
