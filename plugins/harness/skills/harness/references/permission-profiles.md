...
|-------|---------|-------------|---------|---------|
| **A. 서버 측 toolset 필터** | MCP 서버의 `--toolsets` / env (예: `GITHUB_TOOLSETS=issues,prs`) | ⚠️ 진짜 줄어듦 *추정* — server-side는 미측정 / project-scope + user-scope `enabledMcpjsonServers` + `hasTrustDialogAccepted` 모두 *✗ 영향 0* (13·15차 closure, `-p` 모드) [^A] | ✅ 미advertise = 호출 불가 | `.mcp.json` 등록 시 |
