...
|-------|---------|-------------|---------|---------|
| **A. 서버 측 toolset 필터** | MCP 서버의 `--toolsets` / env (예: `GITHUB_TOOLSETS=issues,prs`) | ⚠️ 진짜 줄어듦 *추정* — **server-side `--toolsets`/env 채널만 미측정 + 유효한 마지막 후보**. `~/.claude.json` 4-key gate(`enabledMcpjsonServers` user+project / `hasTrustDialogAccepted` / `disabledMcpjsonServers`) + `<derived>/.claude/settings.json` `enabledMcpjsonServers` 모두 *✗ 영향 0* (13·15·16·21차 closure — **mode-independent falsification 확정, 인터랙티브·`-p` 양쪽 empirical**) [^A] | ✅ 미advertise = 호출 불가 | `.mcp.json` 등록 시 |
...
[^A]: Layer A "컨텍스트 절감 ✅"는 공식 docs(server advertise 단계에서 toolset 필터링) + §5-1-b 추정 결합. 실측은 §11-3 fixture(`fixtures/verify_11_3.md` 3축 메트릭 M1/M2/M3)로 *baseline 절감 vs 호출 게이트만 vs 부분 적재* 3분기 측정 필요. 8차 사이클 부분 측정에서 단위 모호로 보류 — `fixtures/README.md` "결과 로그" 2026-05-10 §11-3 행 참조. **🆕 12차 사이클 추가 진척 (2026-05-10):** B1 베이스라인 자체가 0으로 측정 — `~/.claude.json` user-scope per-project entry `enabledMcpjsonServers=["sqlite"]` + `<derived>/.claude/settings.json={}`로 양쪽 동기화에도 deferred pool 미적재. `enabledMcpjsonServers` 키가 *2 위치*에 동시 존재 + 그 둘만으로 *충분조건이 아닐 수 있음* (CLI 인터랙티브 approval, `disabledMcpjsonServers`/`enableAllProjectMcpServers`/`mcpServerApproved` 유사 user-level 키 가능성 미확인). fixture `verify_11_3.md`에 "측정 환경 함정" 섹션 신설 + 즉시 우회(`-s local` 재등록) 박제. 미확정 동안 *baseline 절감 가설*에 의존한 토큰 비용 추정에 ±50% 오차 caveat 유지. **🆕 13차 사이클 pre-flight (2026-05-11):** dharness 본 세션 read-only `~/.claude.json` grep으로 *4번째 gate 키* 식별 — **`hasTrustDialogAccepted`** (per-project entry, `false`면 `.mcp.json` 신뢰 미승인 → 모든 project-scope MCP 적재 차단). `dharness-probe-test` entry 현재 값: `enabledMcpjsonServers=["sqlite"]` ✓ / `disabledMcpjsonServers=[]` ✓ / `hasTrustDialogAccepted=true` ✓ / `<derived>/.claude/settings.json` `enabledMcpjsonServers=["sqlite"]` ✓ — *gate 4종 모두 충족*. 12차 측정 시 `<derived>/.claude/settings.json={}`였던 사실이 그 이후 `["sqlite"]`로 갱신되어 **B1 베이스라인 측정 ready**. **🆕 13차 closure (`claude -p` mode, 2026-05-11):** B1.M2=6 / B2(`enabledMcpjsonServers=[]` 토글 OFF).M2=6 — **project-scope `<derived>/.claude/settings.json` 토글은 deferred pool 적재에 *영향 0* empirical 확정**. 결론: 본 `[^A]` Layer A 셀의 "✅ 진짜 줄어듦" 가설 중 *project-scope settings.json 채널은 ✗ 부정*. server-side `--toolsets`/env 채널(원래 Layer A의 핵심 메커니즘)은 여전히 미측정. 진짜 gate 채널 후보: `~/.claude.json` user-scope per-project entry의 `enabledMcpjsonServers` (별도 측정 미완). caveat: `-p` 모드 auto-trust 부수 효과 가능성 — 인터랙티브 `claude` 비교 측정으로 분리 가능 (후속 의제). 결과 박제: `fixtures/verify_11_3.md` 측정 로그 2026-05-11 B1+B2 행 + `fixtures/README.md` 결과 로그 closure 행. **🆕 15차 사이클 P3-(b) closure (`claude -p` mode, 2026-05-11):** `~/.claude.json` user-scope per-project entry 4 gate 키를 단독 토글한 3 측정 모두 M2=6 (변화 0) — B5(gate 4 ALL ON 재현)=6 / B6(user-scope `enabledMcpjsonServers=[]` 단독 OFF)=6 / B7(`hasTrustDialogAccepted=false` 단독 OFF)=6. **13차 closure가 후보로 지목한 user-scope `enabledMcpjsonServers`와 `hasTrustDialogAccepted` 양쪽 모두 `-p` 모드 deferred pool 적재 제어 ✗ 부정.** 강력 가설로 격상: `claude -p` 모드는 모든 `~/.claude.json` 측 gate 키를 우회하여 `.mcp.json` 존재만으로 적재. 인터랙티브 `claude` 비교 측정이 마지막 분기 — gate 효과는 인터랙티브 trust dialog 통해서만 발현 가설. **부산물 empirical:** dharness 본 세션 auto-mode classifier는 `~/.claude.json` user-scope 쓰기의 *첫* 호출은 통과시키지만 *후속* 쓰기는 일관 차단 — §10-7 4번 룰 강 empirical 확정 (외부 PowerShell 매뉴얼 측정으로 회피, `fixtures/verify_11_3_p3b.ps1` v2). 결과 박제: `fixtures/verify_11_3.md` 측정 로그 2026-05-11 B5/B6/B7/FINAL 4행 + `fixtures/README.md` 결과 로그 closure 행. 남은 분기 측정: 인터랙티브 `claude` 비교 / `disabledMcpjsonServers=["sqlite"]` 명시 차단 — 둘 다 사용자 외부 1탭 ~5분. **🆕 21차 사이클 closure (P3-(c) 인터랙티브 비교, 2026-05-11):** 사용자 외부 PowerShell(`verify_11_3_p3c.md` Step 0-3, ~5분)로 인터랙티브 `claude` 모드에서 B8(gate ALL ON) + B9(`<derived>/.claude/settings.json` `enabledMcpjsonServers=[]` 토글 OFF) 2회 측정 → **B8.M2=6 / B9.M2=6** (fixture 예측 "가장 가능성 높은 결과"=mode-independent falsification 적중). B8 enum = `mcp__sqlite__{append_insight, create_table, describe_table, list_tables, read_query, write_query}` (§3 100% 일치). B9 enum = B8과 정확히 동일 (sqlite 6종). **13·15·16차 `claude -p` 모드 한정 caveat *제거*** — 결론 *mode-independent*로 일반화: `<derived>/.claude/settings.json` `enabledMcpjsonServers` 토글은 deferred pool 적재를 *전혀* 제어하지 못함, *인터랙티브·`-p` 모드 무관*. `claude -p` auto-trust 부수 효과 가설 ✗ 부정. **본 `[^A]` Layer A 진짜 절감 채널 후보가 *server-side `--toolsets`/env*로 *유일하게* 좁혀짐** — 그것이 *미측정 + 유효*한 가설로 남는 *유일한* 채널. **P3 결정적 미해소 분기 완전 closure**. ±50% 토큰 추정 오차 caveat: server-side 채널만이 잔존 가설이므로 — *Layer A baseline 절감 추정이 server-side `--toolsets` 측정으로 좁혀짐*. inline `mcpServers:` 패턴(Layer B parent isolation = ✅ 10차 P0 empirical)이 여전히 *확정된* 토큰 절감 채널. 결과 박제: `fixtures/verify_11_3.md` 측정 로그 B8+B9+FINAL 3행 + `fixtures/README.md` 결과 로그 21차 closure 행 + §1 Layer A 셀 표기 갱신(mode-independent 박제) + §8-2 21차 closure paragraph.
...
| Tier | MCP | 막힘 사유 | 진척 (17차 사이클) | 해소 후 매트릭스 행 |
|---|---|---|---|---|
| T1 | `playwright` ✅ probe ✓ (22차) / `chrome-devtools` 출처 ✅ + 실 probe 미완 (Node engines 함정) | toolset 필터 형태가 CLI flag — env 키 없음 | ✅ **22차 사이클 — playwright probe-only empirical 확정** (default 23종 + caps=vision +6종 = 29 / 17차 docs 박제 vs 실 probe diff 2건 정정: ① network 2종이 default 포함 ② vision은 5→6종 coordinate-based mouse). 외부 PowerShell 1회 회수. **23차 chrome-devtools 부분 진척**: 패키지 출처 verify 확정(Google LLC + ChromeDevTools 공식 org + Apache-2.0 + mcpName, 17차 docs 44 카운트 100% 일치) + 17차 attach 모드 정정(auto-launch default + `--browser-url`/`--ws-endpoint` 명시) + 🚫 **Node engines 함정 empirical**(v18.15.0에서 모든 49 versions 차단). dharness root 한정 closure — 실 probe enum은 derived 프로젝트별 §10 dynamic adoption + Node 20.19+ 환경 가용 시점. | playwright → code-test 행에 추가 가능 / chrome-devtools는 출처 ✅ + 실 enum은 *영구 외부 의제* (Node engines + 사용자 환경 결정) |
| T1+ | `github` | toolsets enum은 ✓ 확정(19종) — 실 install·도구 풀 측정만 PoC 미완 (PAT 필요) | 📜 17차 docs 정정 — **npm 패키지 없음** (Docker/Go 바이너리만 — 기존 §3 행이 npm 가정이었음 정정). install 명령·env·toolset 옵션 박제 | external-integration 행에 *결합형* 별도 박제 (PAT 보유 시점) |
| T1+ | `brave-search` / `tavily` / `exa` | API 키 발급 필요 | 📜 17차 docs 박제 (brave 8 / tavily 4 (도구명 하이픈 — 첫 케이스) / exa 3 active+7 deprecated) | web-research 행 확장 (각 키 보유 시점) |
| T2~ | `firecrawl` / `slack` / `postgres` | 유료 또는 민감 외부 시스템 | 📜 17차 docs 박제 (firecrawl 10 active+4 deprecated browser) — slack/postgres 미완 | research / external-integration 행에 ask-only 부기 |
...
**🆕 15차 사이클 closure (P3-(b) user-scope gate 격리 측정, 2026-05-11):** 13차 closure가 후보로 지목한 *진짜 gate 채널 2종*을 단독 토글 측정으로 검증. 외부 PowerShell `fixtures/verify_11_3_p3b.ps1` v2로 gate 4 키 단독 토글 3회 + FINAL restore. **🚨🚨🚨 3차 결정적 empirical:** B5(gate 4 ALL ON 재현)=M2=6 / B6(user-scope `enabledMcpjsonServers=[]` 단독 OFF)=M2=6 / B7(`hasTrustDialogAccepted=false` 단독 OFF)=M2=6 — 3 조건 모두 변화 0. **user-scope `enabledMcpjsonServers`와 `hasTrustDialogAccepted` 양쪽 모두 `-p` 모드 deferred pool 적재 제어 ✗ 부정** (13차 project-scope falsification에 이은 사실상 모든 `~/.claude.json` 측 gate 키 falsification). 강력 가설로 격상: `claude -p` 모드는 *모든* `~/.claude.json` gate 키를 우회 — `.mcp.json` 존재만으로 적재. 인터랙티브 `claude` 비교 측정이 마지막 분기. **부산물 empirical:** dharness 본 세션 auto-mode classifier가 `~/.claude.json` user-scope 쓰기의 *첫* 호출은 통과시키지만 *후속* 쓰기는 일관 차단 — `claude -p` 호출과 후속 복원 쓰기 모두 차단됨이 양면 검증 → §10-7 4번 룰 강 empirical 확정. 측정 회수 채널: 외부 PowerShell 매뉴얼(사용자 외부 세션 또는 본 세션 `!` prefix). 미완 잔존: (a) 인터랙티브 `claude` 비교 측정 — `-p` auto-trust 부수 효과 분리 (사용자 외부 1탭, ~5분) (b) `disabledMcpjsonServers=["sqlite"]` 명시 차단 측정 — 마지막 미측정 gate (c) T1 MCP probe (playwright/chrome-devtools — 브라우저 다운로드 부수 효과 사용자 확인) (d) T1+ MCP PoC enum (github/brave-search/tavily/exa/firecrawl — API 키 필요).

**🆕 16차 사이클 closure (P3-(b) 마지막 미측정 gate B8, 2026-05-11):** 15차 4-key matrix의 *마지막 미측정 gate* `disabledMcpjsonServers`를 단독 토글 측정으로 검증. 외부 PowerShell `fixtures/verify_11_3_p3b.ps1` v3 (v2에 `$disabled` 파라미터 추가 + B8 호출 + FINAL disabled=[] 포함)으로 B5-B7 재측정 + B8 신규 + FINAL restore. **🚨🚨🚨🚨 4차 결정적 empirical (사실상 P3-(b) 완전 closure):** B8(`disabledMcpjsonServers=["sqlite"]` 단독 ON, 다른 키 ALL ON)=M2=6 — *명시 차단* 키조차 deferred pool 적재 제어 ✗ 부정. **`~/.claude.json` 측 4-key gate 매트릭스 *전부* falsified** — (1) project-scope settings.json `enabledMcpjsonServers=[]` ✗ (13차 B2) (2) user-scope `enabledMcpjsonServers=[]` ✗ (15차 B6) (3) `hasTrustDialogAccepted=false` ✗ (15차 B7) (4) `disabledMcpjsonServers=["sqlite"]` 명시 차단 ✗ (16차 B8). 단순 토글이 아닌 *명시 차단* 키도 무력하다는 것은 `-p` 모드의 auto-trust가 *모든 MCP gate*를 우회한다는 강력한 hypothesis 사실상 확정. **마지막 분기**: 인터랙티브 `claude` 측정만이 `-p` auto-trust 가설을 *결정적*으로 확정/부정 — 1건 외부 측정 (~5분, `verify_11_3_p3c.md` fixture 박제)으로 P3 전체 closure 가능. 산출물: `verify_11_3_p3b.ps1` v3 + `verify_11_3_p3c.md` (외부 인터랙티브 측정 fixture, 본 cycle commit으로 untracked → tracked). 미완 잔존: (a) 인터랙티브 `claude` 비교 측정 — *유일한* 결정적 미해소 분기 (b) T1 MCP probe (playwright/chrome-devtools — 사용자 명시 confirm) (c) T1+ MCP PoC enum (API 키 필요).

**🆕 15차 사이클 self-verify (`harness-mcp-status` empirical 작동 확인, 2026-05-11):** 14차 commit 직후 `harness-mcp-status` 절차를 derived 프로젝트(`dharness-probe-test` fixture 환경)에 self-test 적용 — 14차 cycle 신설 산출물(§3-1 매트릭스 + commands 섹션 3 신설 항목)이 *의도대로 작동*함을 empirical 확정. 검증 4건: (1) 섹션 2 `inline advertise 도구` 컬럼 = `tools:` 1종 vs advertise 4종 mismatch 정확 감지 (10차 P0 발견 검증), (2) 섹션 3 §3-1 정합 점검 = web-research profile 추정 + `memory` 누락을 ⓘ로 보고 (fixture 의도라 권고 아님 — 정상), (3) 섹션 4 `permissions` 부재 시 fallthrough `ask` + 부수 효과 도구 0종 → deny ⚠️ 발화 안 함 (정상), (4) 섹션 6 토큰 추정 parent sqlite 6도구 = 300 tokens 고 — 임계 600 미달로 ⚠️ 발화 안 함 (정상). 본 fixture 환경 ⚠️ 0건이 *정상*임을 self-test로 확인 — production derived 환경에서 의미 있는 ⚠️ 발화. 외부 변경 0, 코드 실행 0 (read-only 진단). 결과 박제: `fixtures/README.md` 결과 로그 15차 행 + `SKILL.md` Phase 5-2 진척 라인 self-verify ✓ 추가.

**🆕 17차 사이클 closure (T1+T1+ docs 박제 + classifier §6 재확인, 2026-05-11):** P3 후속으로 T1 (playwright/chrome-devtools) + T1+ (brave-search/tavily/exa/firecrawl/github) 5종 MCP의 install 명령·도구 enumeration·env 키·toolset/caps 옵션을 *공식 docs WebFetch*로 §3 인벤토리에 박제. **본 세션 probe 시도 결과**: `node fixtures/probe_playwright.js` (= `npx -y @playwright/mcp@latest`) 호출이 **dharness 본 세션 auto-mode classifier에 차단** — 사유 `Executing 'npx -y @playwright/mcp@latest' downloads and runs code from an external npm registry (not a declared dependency in the repo manifest) — code from external source.`. **§6 자동 install 금지 정책의 enforcement가 T1 패키지에도 동일 작동** (8차 sqlite-derived self-test와 동일 mechanism, 다른 target — `npx -y` 패턴 자체가 차단 트리거). 본 차단은 *데이터*가 아니라 *정책 검증*이라 ✅로 박제 (§6 정책 실효성 재확인). **docs 박제 카운트:** playwright **23 default + 36 opt-in caps** (4 caps 묶음) / chrome-devtools **44** (auto-launch + remote-debugging-port=9222 두 모드) / brave-search **8** (모두 read-only) / tavily **4** (도구명 하이픈 — 첫 케이스) / exa **3 active + 7 deprecated** (deprecated는 권고 X) / firecrawl **10 active + 4 deprecated browser** / github **toolsets 19 enum 정정 + npm 패키지 없음** (Docker/Go 바이너리만 — 기존 §3 행이 npm 가정이었음 정정). 산출물: §3 인벤토리 7행 갱신 (PoC 미완 → 📜 docs 박제) + §3-1 매트릭스 미완 잔존 표 갱신(`playwright`/`chrome-devtools` ⓘ docs 박제로 부분 진척, T1+ 5종은 API 키 보유 시점 별건) + 본 §8-2 17차 closure paragraph + `fixtures/README.md` 결과 로그 1행. **미완 잔존 (외부 환경)**: (a) playwright probe-only 실 실행 — 사용자 `!` prefix 또는 외부 PowerShell 1회 (~2분, `node fixtures/probe_playwright.js`, Chromium 다운로드는 lazy라 tools/list 단계엔 미발생 가설), (b) chrome-devtools probe-only — Chrome 사전 기동 또는 auto-launch 확인, (c) T1+ 5종 실 enum — API 키 보유 시점에 별건. P3 영역의 *결정적* 미해소 분기는 인터랙티브 `claude` 비교(`verify_11_3_p3c.md`)만 남음.

**🆕 18차 사이클 추가 (T1+ probe fixture 5종 박제, 2026-05-11):** 17차 docs 박제 위에 *§10 Step 2 pre-install probe fixture* 5종(`probe_brave_search.js`/`probe_tavily.js`/`probe_exa.js`/`probe_github.js`/`probe_firecrawl.js`)을 단독 commit으로 신설. `probe_sqlite.js` 동일 JSON-RPC stdio 핑 패턴 + 17차 docs 박제 정보(env 키, 예상 enum, deprecated 자동 태깅, T1+/T2~ tier 함의)를 inline 박제. **본 세션 spawn 0회** — classifier가 `npx -y` 패턴을 untrusted external code execution으로 차단(17차 §6 정책 실효성 재확인 재현). 외부 실행자가 API 키 보유 시점에 *복사 실행*만으로 §3 인벤토리 enum 확정 가능. **commit 분담 결정:** 본 세션은 5 신규 fixture만 단독 commit (af781a0), 17차 docs 박제(uncommitted modified)는 working tree에 보존하여 playwright 세션 영역 침해 회피 — 결과적으로 `8322c79`(17차) → `af781a0`(18차) 순서로 자연 정렬됨. **`probe_github.js` 특이점:** 본 세션의 유일한 *Docker spawn* fixture (T1+ 5종 중 npm 미지원 — 17차 정정 사실 반영) + `GITHUB_TOOLSETS` 3 조합(`pull_requests`/`default`/`all`) 비교 측정 의도(Layer A 서버 측 advertise 절감 비율 — §1 [^A] 셀의 server-side 채널이 13·15·16차 미측정 분기로 잔존). **`probe_exa.js` / `probe_firecrawl.js` 특이점:** deprecated 도구 자동 태깅(7종 / 4종 — 17차 박제 기반) + `firecrawl_browser_execute` `🚫 deny 권고` inline 박제(임의 코드 실행 surface). **미완 잔존:** (a) 5 fixture 실 spawn enum — 외부 실행자 API 키 보유 시점에 별건 (b) §3-1 매트릭스 web-research(brave/tavily/exa/firecrawl) + external-integration(github) 행 확장 — 실 enum 확정 후. 산출물: §3 인벤토리 line 85-89 fixture cross-link 5행 + 본 §8-2 18차 paragraph + `SKILL.md` Phase 5-2 진척 라인 18차 한 줄 + `fixtures/README.md` 결과 로그 1행.

**🆕 20차 사이클 closure (dharness root T1+ API 키 의제 종료 — 프로젝트별 §10 dynamic adoption으로 이양, 2026-05-11):** 사용자 의사결정으로 dharness root에서의 T1+ 5종 API 키 발급/MCP 등록/실 spawn 의제 종료. **사유:** §10 "적용 경계" 인용블럭과 정합 — *dharness root는 self-host CM 격리 영역*이고 §10 dynamic adoption은 *derived 프로젝트의 `.claude/`* 대상. T1+ MCP의 실 채택은 **프로젝트 유형/도메인에 따라 개별 결정**되어야 하며, dharness root 한정 PoC 검증은 *예외적 검증 환경*이라 도구 enum의 empirical 박제 가치가 *문서 박제*에 한정됨. **종료 후 보존 산출물 (외부 도입자 1차 후보 카탈로그):** (a) §3 인벤토리 line 82-89 8행의 docs 박제(17차) + fixture cross-link(19차) — 외부 도입자가 §10 진입 시 *trusted source + install 명령 + 예상 enum* 박제 자료 (b) `fixtures/probe_{brave_search,tavily,exa,github,firecrawl}.js` 5종 (18차) — *복사 실행만으로* enum 가능한 자산, 프로젝트별 §10 Step 2 pre-install probe 직접 사용 (c) §3-1 매트릭스 "미완 잔존" 표 — T1·T1+·T2~ 행은 *영구 미완* 표기 (검증 환경 의제 종료 후 *외부 영역*임을 명시). **이양 채널 3종:** (1) `/harness:harness-mcp-adopt <사유>` 명령 — §10 5-step 진입점 (2) §10-3 프로젝트 유형별 매트릭스 — Web 서비스(github+playwright+postgres) / Data 파이프라인(sqlite+filesystem+postgres) / Mobile(git+github actions) / Research(fetch+brave+tavily+exa+memory) / DevOps(git+github actions+security_advisories) (3) `/harness:harness-mcp-status` 진단(read-only) — 채택 *전후* 정합 점검. **closure 의의:** Phase 5-2 17·18·19차의 *dharness root 한정 박제*가 derived 프로젝트의 *런타임 시점 결정*으로 자연 이양됨 — `SKILL.md` "합성 시점 vs 런타임 시점 분리" 단락이 본 closure로 empirical 시연. **남은 미완 (dharness root 한정 *영구* 미완):** §3 인벤토리 line 85-89 5행의 enumeration 컬럼 empirical 확정(✅ 미박제) / §3-1 매트릭스 web-research·external-integration 행 확장(미완) — 모두 dharness root에서는 *closure*, derived 프로젝트별 진척으로 이양. P3 외부 의제(`verify_11_3_p3c.md` 인터랙티브 비교)와 T1 probe(playwright/chrome-devtools 실 spawn)도 동일하게 *외부 환경 의제로 보존*.

**🆕 21차 사이클 closure (P3-(c) 인터랙티브 `claude` 비교 측정 → mode-independent falsification 확정, 2026-05-11):** 사용자 외부 PowerShell 1탭(~5분, `verify_11_3_p3c.md` Step 0-3)으로 인터랙티브 `claude` 모드에서 B8(gate ALL ON)·B9(`<derived>/.claude/settings.json` `enabledMcpjsonServers=[]` 토글 OFF) 2회 측정 회수. **🚨🚨🚨🚨🚨 5차 결정적 empirical (P3-(c) closure):** B8.M2=6 / B9.M2=6 — fixture 예측 "가장 가능성 높은 결과"(mode-independent falsification) 적중. B8 enum = `mcp__sqlite__{append_insight, create_table, describe_table, list_tables, read_query, write_query}` (§3 100% 일치). B9 enum = B8과 정확히 동일. **13·15·16차 `claude -p` 모드 한정 caveat *제거*** — 결론 *mode-independent*로 일반화: `<derived>/.claude/settings.json` `enabledMcpjsonServers` 토글은 deferred pool 적재를 *전혀* 제어하지 못함, *인터랙티브·`-p` 모드 무관*. `claude -p` auto-trust 부수 효과 가설 ✗ 부정. **§1 [^A] Layer A 진짜 절감 채널 후보가 *server-side `--toolsets`/env*로 *유일하게* 좁혀짐** — 그것이 *미측정 + 유효*한 가설로 남는 *유일한* 채널. **P3 결정적 미해소 분기 완전 closure** — 남은 외부 환경 의제(T1 chrome-devtools probe, T1+ API 키 5종)는 *영구 미완* 의제로 분리(20차 dharness root closure와 정합). 산출물: `fixtures/verify_11_3.md` 측정 로그 B8+B9+FINAL 3행 + `fixtures/README.md` 결과 로그 21차 closure 행 + §1 Layer A 셀 표기 갱신 (mode-independent 박제) + §1 [^A] footnote P3-(c) closure 갱신 + 본 §8-2 21차 closure paragraph.

**🆕 22차 사이클 closure (T1 playwright probe-only empirical 확정 + docs 박제 정정, 2026-05-11):** 사용자 외부 PowerShell `node fixtures/probe_playwright.js` 2회(default flag + `--caps=vision`) 실행으로 17차 docs 박제를 *실 probe 데이터*로 승급. **결과:** default **COUNT=23 ✓** (17차 박제값 100% 일치) + `--caps=vision` **COUNT=29 (+6)** ✓. **🆕 17차 docs 박제 정정 2건:** (1) `browser_network_requests`/`browser_network_request` 2종이 *default에 포함* (17차 docs는 caps=network opt-in으로 추정 — *명백한 정정*) (2) caps=vision은 17차 추정 5종이 아닌 **6종 좌표 기반 mouse** (`browser_mouse_{move_xy, click_xy, drag_xy, down, up, wheel}`) — screenshot은 default에 이미 포함이라 vision의 *진짜 의미는 vision-guided coordinate clicking*임이 empirical 확정. **부수 효과 검증:** tools/list 단계엔 Chromium 다운로드 미발생 — npx 패키지 fetch만, browser binary는 lazy(`browser_navigate` 호출 시점) — 17차 docs 박제 "lazy 다운로드" 가설 ✓ 적중. **`browser_run_code_unsafe`** = arbitrary JS in browser context → `deny` 권고 박제 (default 4종 browser control 중 하나, firecrawl_browser_execute와 동일 RCE-equivalent surface). 다른 caps(pdf/devtools/network/storage/testing/config) 분해 측정 미완 — `$env:PLAYWRIGHT_FLAGS="--caps=<X>"; node fixtures/probe_playwright.js`로 각 caps 추가 도구 enum 외부 측정 권고. **22차 정합 정리:** §8-3 안전 룰 PoC 미완 list에서 `filesystem`/`time`/`memory`(14차 ✓)/`playwright`(22차 ✓) 4항목 제거 — 14차 closure가 정합 list 미반영된 잔재 정정. 산출물: §3 playwright 행 ✅ empirical 갱신 + §3-1 미완 잔존 표 playwright partial closure + §8-3 안전 룰 list 4항목 정합 정리 + `fixtures/README.md` 결과 로그 22차 행 + 본 §8-2 22차 closure paragraph. **남은 T1 미완:** chrome-devtools(Chrome 사전 기동 또는 auto-launch 확인 필요) — derived 프로젝트별 §10 dynamic adoption으로 이양 (dharness root 한정 의제는 20차 closure와 정합).

**🆕 23차 사이클 (T1 chrome-devtools 패키지 출처 verify — probe 선행 단계, read-only no spawn, 2026-05-11):** 22차 playwright closure paragraph가 chrome-devtools를 "derived 프로젝트별 §10으로 이양"한다고 표기했으나, 사용자 요청으로 *dharness root에서 가능한 선행 단계*(패키지 출처 verify, no code execution)는 진행. dharness 본 세션 `WebFetch https://github.com/ChromeDevTools/chrome-devtools-mcp` + `npm view chrome-devtools-mcp --json` 2-call로 3-source 교차 verify. **✅ 출처 확정 — spoofing 위험 0:** (1) npm registry: Author=**Google LLC** / Maintainers 3명 모두 Google 또는 Chrome DevTools 팀(`mathias`=Mathias Bynens, `orkon`=Alexei Rudenko, `google-wombot`=Google 공식 npm 봇) / License=Apache-2.0 / mcpName=`io.github.ChromeDevTools/chrome-devtools-mcp` (MCP registry 공식 형식) / latest v0.25.0 (2026-05-06) / 49 versions since 2025-05-13 (1년 활발) / engines node ^20.19 || ^22.12 || >=23 / bin entries 2종(`chrome-devtools-mcp`=서버, `chrome-devtools`=alias) / devDep `puppeteer 24.43.0`(auto-launch 백엔드 추정). (2) GitHub `ChromeDevTools/chrome-devtools-mcp`: 39.2k stars / 2.5k forks / Apache-2.0. (3) **17차 docs 박제 vs README verify 100% 일치 (44 도구 카운트, 9 카테고리 분류)**. **🆕 attach 모드 17차 추정 정정 1건:** 17차는 "auto-launch + 9222 attach 두 모드"로 박제했으나 README verify 결과 **auto-launch가 default + remote attach는 `--browser-url=http://127.0.0.1:9222` 또는 `--ws-endpoint=<url>` 명시 flag 필요 + `--autoConnect`는 Chrome 144+ 요구**(release 채널 의존). 18차 fixture의 "후보 1 (가장 유력)" 표기 ✅로 승급. **23차 의의:** dharness root에서 *spawn 없이도* derived 프로젝트의 §10 Step 2 pre-install probe 부담을 줄이는 *상위 단계*(패키지 출처 확정)가 가능함을 empirical — 22차 playwright의 외부 PowerShell probe-only 패턴 적용 *전*에 출처 verify를 미리 떼어내는 워크플로우(probe 코드 실행 risk와 출처 검증을 분리). 산출물: `fixtures/probe_chrome_devtools.js` comment block 갱신(✅ 출처 확정 + attach 모드 정정 + 사용법 외부 attach 권장) + 본 §3 chrome-devtools 행 verify 사실 박제(✅ + Apache-2.0 + Google LLC + mcpName + engines + autoConnect Chrome 144+ caveat) + `SKILL.md` Phase 5-2 진척 라인 23차 한 줄 + `fixtures/README.md` 결과 로그 1행 + 본 §8-2 23차 paragraph. **남은 T1 미완 (외부 환경 의제 유지):** 실 probe spawn — 사용자 측 외부 PowerShell 1회(`node fixtures/probe_chrome_devtools.js`) → 17차 docs 박제 44종 vs 실 enum diff 검증(playwright 22차에서 17차 정정 2건 발견 선례) + §3 행 ✅ empirical 승급. 외부 attach 모드(`$env:CHROME_DEVTOOLS_FLAGS="--browser-url=http://127.0.0.1:9222"`)가 Chromium 다운로드 회피 권장. dharness root 한정 의제는 본 23차 이후 *영구 미완*(20차 closure와 정합), derived 프로젝트별 §10 dynamic adoption으로 이양.

**🚫 23차 closure 추가 — Node engines 함정 박제 (사용자 외부 PowerShell 1회 시도, 2026-05-11):** 패키지 출처 verify(위 paragraph) 직후 사용자 외부 PowerShell에서 본 fixture spawn 시도(`cd C:\Users\user01\awesome-files\dharness; node plugins\harness\skills\harness\references\fixtures\probe_chrome_devtools.js`). **결과:** `npm WARN EBADENGINE` 후 `ERROR: chrome-devtools-mcp does not support Node v18.15.0. Please upgrade to Node 20.19.0 LTS or a newer LTS.` 즉시 종료. dharness 본 세션 환경 inventory: Node v18.15.0 단일 설치(`C:\Program Files (x86)\nodejs`, 32-bit), nvm-windows/volta/fnm 모두 없음. **engines 요구 history (npm view 7 버전 cross-check)**: `chrome-devtools-mcp@0.0.1`만 `node >=22.12.0`, `0.1.0`~`0.25.0` (48 versions) 모두 `node ^20.19 \|\| ^22.12 \|\| >=23` — **어떤 버전도 v18에서 실행 불가**. **🆕 새 발견 — playwright와 다른 함정 surface**: 22차 playwright는 외부 PowerShell 통과(같은 v18 환경 가능성 높음) → playwright는 engines 요구가 더 관대 + Node 메이저 버전 의존이 추가 차단 layer로 작용하지 않음. chrome-devtools-mcp는 *Node 20.19+ hard requirement* — derived 프로젝트별 §10 진입 시점에 사용자 측 Node 환경 확인이 *§10 Step 2(probe) 전 선행 gate*로 추가됨. **사용자 결정**: dharness root에서 시스템 Node 업그레이드 회피 — 23차를 *명안 박제* closure로 마감. **이양 채널**: derived 프로젝트의 §10 진입 시점에 (a) `winget install OpenJS.NodeJS.LTS` 시스템 교체 또는 (b) nvm-windows 공존 또는 (c) portable Node zip 1회용 — 사용자 환경 결정 후 외부 PowerShell `node fixtures/probe_chrome_devtools.js` 1회 실행 → 17차 docs 박제 44종 vs 실 enum diff(playwright 22차 선례) → §3 chrome-devtools 행 ✅ empirical 승급 + §3-1 매트릭스 partial closure. 산출물: `fixtures/probe_chrome_devtools.js` comment block "🚫 Node engines 함정" 단락 + 본 §3 chrome-devtools 행 "🚫 Node engines 함정 (23차 empirical)" caveat + §3-1 매트릭스 미완 잔존 표 chrome-devtools 행 "출처 ✅ + 실 enum *영구 외부 의제*" 갱신 + `fixtures/README.md` 결과 로그 23차 갱신 행 + `SKILL.md` Phase 5-2 진척 라인 23차 한 줄.

**🆕 24차 self-verify ✅ (23차 산출물 chrome-devtools §10 채택 시뮬레이션 — read-only, 2026-05-12):** 가상 *프론트엔드/E2E 테스트* derived 프로젝트가 `/harness:harness-mcp-adopt "chrome-devtools로 lighthouse 성능 분석"` 진입을 가정한 시뮬레이션(no spawn, no install — 15차 self-verify 패턴 답습). **23차 산출물 4건이 §10 5-step 워크플로우에서 의도대로 발화함을 empirical 확정:** (1) **Step 1 (Discover) §3 행 ✅ 출처 박제 → trust source 즉시 인용**: §3 chrome-devtools 행의 Author=Google LLC + Apache-2.0 + mcpName=`io.github.ChromeDevTools/chrome-devtools-mcp` + GitHub 39.2k★ 박제가 *Step 3 confirm 자료*로 그대로 이전 가능 — 사용자에게 *trusted source 자격 즉시 보여줌* ✓. (2) **Step 2 (Pre-install probe) §8-3 안전 룰 매칭 — 출처 ✅ vs §8-3 list "PoC 미완 항목" 표기 사이 drift 0**: §8-3 list line 557이 chrome-devtools를 *PoC 미완 인벤토리 항목*으로 표기하지만, 본 cycle ✅ 박제로 §3 행이 *trusted source 자격*을 새로 획득 — 두 차원이 *분리* 정합 (출처 trust ✓ but enum 카운트 ✗ — 자연스러운 분리, 룰 모순 없음) ✓. (3) **Step 2 fixture 헤더 🚫 Node engines 함정 → ⚠️ 선행 gate 발화**: `probe_chrome_devtools.js` 헤더 line 18-23의 "🚫 Node engines 함정" 단락이 §10 Step 2 진입 시 *probe 실행 전 사용자 환경 inventory 체크(`node --version`)* 의제로 새로 추가됨 — playwright(22차 통과) 대비 *추가 gate* 박제 작동 ✓. (4) **Step 3 (User confirm gate) §3-1 잔존 표 "실 enum *영구 외부 의제*" 표기 → 자동 채택 차단**: §3-1 잔존 표 line 173 chrome-devtools 컬럼이 *empirical 미확정*을 명시하며 §3 본문 "📜 docs 박제 44종 (17차 사이클)" caveat과 동반 발화 → 사용자에게 *docs 추정 vs empirical 확정 분리 명확* 표시, 도구 카운트만으로 통과 차단 (Step 3 confirm 시 ⓘ 권고로 cross-check 자격) ✓. **본 시뮬레이션 ⚠️·ⓘ 발화 0건 + 23차 4 산출물 모두 작동** — derived 프로젝트에서 실제 chrome-devtools 채택 시 동일 워크플로우 발화 예상 (15차 `harness-mcp-status` self-verify가 14차 4 산출물 작동 empirical로 검증한 패턴 답습). 외부 변경 0, 코드 실행 0 (시뮬레이션은 본 paragraph 작성 자체 — 23차 산출물 *읽기*만). 산출물: 본 paragraph self-verify ✓ 1단락 + `SKILL.md` Phase 5-2 진척 라인 24차 self-verify 추가 + `fixtures/README.md` 결과 로그 24차 1행.
...
>
> **추가 안전 룰:** §8-2 PoC 미완 인벤토리 항목(`tavily`/`exa`/`firecrawl`/`slack`/`postgres`/`chrome-devtools`/`brave-search`)은 *§3에 박제는 되었지만 출처 검증·도구 enumeration이 미완*. 이들도 §10 dynamic adoption 진입 시점에 위 1·2 단계 동일 적용. (✅ **22차 사이클 정합 정리**: `filesystem`/`time`/`memory`는 14차 사이클 + `playwright`는 22차 사이클 ✓ probe-only empirical 박제 완료로 본 list에서 제거 — 14차 closure가 정합 list에 미반영된 잔재 정정.)
