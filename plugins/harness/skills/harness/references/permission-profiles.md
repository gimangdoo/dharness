...
|-------|---------|-------------|---------|---------|
| **A. 서버 측 toolset 필터** | MCP 서버의 `--toolsets` / env (예: `GITHUB_TOOLSETS=issues,prs`) | ⚠️ 진짜 줄어듦 *추정* — server-side는 미측정 / `~/.claude.json` 4-key gate(`enabledMcpjsonServers` user+project / `hasTrustDialogAccepted` / `disabledMcpjsonServers`) 모두 *✗ 영향 0* (13·15·16차 closure, `-p` 모드 한정 — `-p` auto-trust 사실상 확정, 인터랙티브 미측정) [^A] | ✅ 미advertise = 호출 불가 | `.mcp.json` 등록 시 |
...
| `playwright` (microsoft/playwright-mcp) | T1 | code-test | `claude mcp add playwright -- npx -y @playwright/mcp@latest [--caps=vision,pdf,devtools,network,storage,testing,config] [--browser=chromium\|firefox\|webkit\|msedge] [--isolated] [--headless]` 📜 docs 박제 (17차 사이클, dharness 본 세션 probe 시도는 auto-mode classifier가 외부 코드 실행으로 차단 — §8-2 17차 재확인) | 📜 docs 박제 23종 (default — 17차 사이클): **navigation/interaction 11종** = `browser_navigate`, `browser_navigate_back`, `browser_click`, `browser_type`, `browser_hover`, `browser_press_key`, `browser_drag`, `browser_drop`, `browser_select_option`, `browser_fill_form`, `browser_file_upload` + **inspect 4종** = `browser_snapshot`, `browser_take_screenshot`, `browser_wait_for`, `browser_console_messages` + **browser control 5종** = `browser_close`, `browser_resize`, `browser_handle_dialog`, `browser_evaluate`, `browser_run_code_unsafe` (⚠️ RCE-equivalent — 항상 `deny` 권고) + **tabs 1종** = `browser_tabs` + **caps 확장**(opt-in): `--caps=network` 6종 / `--caps=storage` 9종 / `--caps=devtools` 9종 / `--caps=vision` 5종 / `--caps=pdf` 1종 / `--caps=testing` 5종 / `--caps=config` 1종. **부수 효과**: 첫 spawn 시 Chromium ~120MB lazy 다운로드 (browser_navigate 호출 시점) — 사용자 명시 confirm 필수 |
...
|---|---|---|---|---|
| T1 | `playwright` / `chrome-devtools` | toolset 필터 형태가 CLI flag — env 키 없음 (PoC enum 미완) | 📜 17차 docs 박제 (playwright 23 default+36 caps / chrome-devtools 44) — 실 probe 미실행 (dharness classifier 차단, 외부 1회 `node fixtures/probe_playwright.js`로 confirm 가능) | code-test 행에 추가 (probe 후) |
