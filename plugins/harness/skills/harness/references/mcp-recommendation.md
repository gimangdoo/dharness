...
Phase 5-2가 합성하는 에이전트 `.md` frontmatter `description:` + 본문 "핵심 역할" 섹션을 입력으로 받는다. LLM이 다음 6 신호를 *추출*만 (강제 분류 X — 다중 매칭 허용).
...
**복합 매핑 허용** — 1 에이전트가 S1+S3+S6 동시 매칭 가능. 매칭된 신호 집합 → §3 후보 풀 검색 키.
...
| schema/필수 인자 박제 미완 (📜 docs only) | -2 |
...
```
R-1. 신호 추출 — §1 6 신호 LLM 추론, 매칭 신호 집합 박제
R-2. R0 검색 — §3·§3-1에서 신호별 후보 grep
