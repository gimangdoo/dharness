...
> **외부 입력 도구 = 가드 의무 트리거.** frontmatter `tools:`에 `Bash`·`Write`·`Edit`·`WebFetch`·`WebSearch`·`PowerShell` 중 하나라도 있으면, 그 에이전트는 외부 데이터(웹 응답·파일 내용·셸 출력·사용자 제공 텍스트·DB 값)를 컨텍스트로 들인다 — 인젝션 표면이 있다. 이 경우 `## 입력 신뢰 경계` 섹션 박제는 **필수**다.
