...
- [스킬 개수 — 흡수·분리 사유 박제](#스킬-개수--흡수분리-사유-박제)
...
스킬 디렉토리를 만들면 `SKILL.md`는 frontmatter만으로 끝나지 않는다 — 아래 5요소를 담은 본문을 **같은 동작에서** 박제한다. 빈 `SKILL.md`나 디렉토리만 남기면 오케스트레이터·에이전트의 스킬 참조가 dangling이 되어 하네스가 즉시 실행 불가다 (SKILL.md Phase 6 "빈 스킬 금지 invariant").

```markdown
---
name: skill-name
description: "하는 일 + 트리거 상황. 적극적('pushy')으로. (§1 참조)"
---

# Skill Name

## 트리거
[언제 이 스킬을 쓰는가 — 자연어 상황 ≥ 1]
...
오케스트레이터 스킬도 같은 골격에 Phase 표·데이터 흐름·에러 핸들링·테스트 시나리오를 더한다. 본문이 50줄을 넘지 못할 만큼 얇으면 그 스킬은 별도 파일 대신 에이전트 정의에 인라인할 후보다 (`agent-design-patterns.md` "스킬 ↔ 에이전트 연결" — 50줄 이하 전용 스킬은 인라인). 즉 독립 `SKILL.md`로 남길 스킬은 인라인 임계를 넘는 실질 본문을 가진다.
...
**모든 스킬은 `## 예시`에 입출력 사례 ≥ 1개를 박제한다 (P1 의무).** 추상 규칙만 있으면 엣지 케이스에서 판단이 흔들린다 — 예시는 긴 설명보다 효과적이다:
...
스킬 개수는 도메인 역량을 어떻게 쪼개느냐에서 도출된다 — 한 역량을 독립 스킬로 분리할 수도, 인접 역량을 한 스킬로 흡수할 수도 있다. 개수 자체에 정답은 없지만, **흡수나 분리를 결정했으면 그 사유를 오케스트레이터 SKILL.md 본문에 한 줄 박제한다.**

- **흡수** (인접 역량을 한 스킬로 묶음): "X·Y는 입출력이 강결합이라 단일 스킬 `xy-skill`로 통합" 형태로 명시.
- **분리** (한 역량을 여러 스킬로 쪼갬): "Z는 독립 트리거·재사용 가치가 있어 별도 스킬로 분리" 형태로 명시.

사유 없는 개수 편차는 합성 회귀 평가에서 일관성 결함 신호다 — 검토자가 누락인지 의도인지 판별할 수 없다. cardinality 판정 논리는 `agent-design-patterns.md` "에이전트 깊이 — deep vs shallow"의 deletion test와 동일 — pass-through 스킬은 인라인 회수, leverage 있는 역량만 독립 스킬로 남긴다.
