...
> - *PoC 진척*: `references/permission-profiles.md` §11 + `references/fixtures/` 4종 reproducer. **§11-1 ✓** (6차) / **§11-3 fixture 본문 갱신** (7차, 3축 메트릭) / **§11-4 e2e ✓** (8차, sqlite 시연으로 §10 5-step 검증) / **§11-2 P0 ✅** (10차, 정정 schema로 양면 empirical 확정 + 🆕 `tools:` allowlist는 inline 서버 도구 통제에 무력 발견) / **§11-3 P1 부분 closure** (13차, `claude -p` mode — project-scope settings.json 토글은 deferred pool 영향 0 부정) / **P2 T0 batch ✅** (filesystem 14 / time 2 / memory 9 — probe-only enum) / **14차 §3-1 매트릭스 박제** (7 T0 MCPs × 4 capability profile 정합 → 합성 시점의 *런타임 가용 default 카탈로그*, web-research 2번째 synthesis_example 동시 박제) / **15차 self-verify ✅** (`harness-mcp-status` self-test로 14차 cycle 산출물 4건 empirical 작동 확인 — 섹션 2 advertise mismatch 감지 / 섹션 3 §3-1 정합 점검 / 섹션 4 fallthrough 룰 / 섹션 6 토큰 임계) / **15차 P3-(b) closure** (`claude -p` mode — user-scope `enabledMcpjsonServers` + `hasTrustDialogAccepted` 양쪽 모두 deferred pool 영향 0 부정 → `claude -p` 모드가 모든 `~/.claude.json` gate 키를 우회한다는 강력 가설 격상, 인터랙티브 비교만 잔존) / **16차 B8 closure** (`disabledMcpjsonServers=["sqlite"]` 명시 차단도 영향 0 — `~/.claude.json` 4-key gate 매트릭스 *전부* falsified, `-p` auto-trust 사실상 확정 / 마지막 분기는 인터랙티브 `verify_11_3_p3c.md` 외부 1회) / **17차 T1+T1+ docs 박제 📜** (WebFetch로 7 MCP 공식 docs enum: playwright 23+caps / chrome-devtools 44 / brave 8 / tavily 4 (도구명 하이픈 첫 케이스) / exa 3+deprecated / firecrawl 10+deprecated / github 19 toolset + npm 패키지 없음 정정 — Docker/Go만; dharness 본 세션 probe 시도는 classifier가 `npx -y` 패턴 차단 → §6 정책 실효성 *T1 패키지에서도 재확인*) / **18차 T1+ probe fixture 5종 박제** (`probe_brave_search`/`tavily`/`exa`/`github`/`firecrawl` — JSON-RPC stdio 핑 패턴 + env 키 placeholder + deprecated 자동 태깅 + `probe_github.js`는 Docker spawn + `GITHUB_TOOLSETS` 3 조합 비교 측정 의도, 본 세션 spawn 0회 → API 키 보유 시점 외부 실행자가 enum 확정; commit 분담 회피: `8322c79`(17차) → `af781a0`(18차) 자연 정렬) / **20차 closure** (사용자 의사결정 — dharness root T1+ API 키/MCP 등록/실 spawn 의제 종료, §10 적용 경계와 정합 = dharness root는 self-host CM 격리, derived 프로젝트의 `.claude/`가 §10 dynamic adoption 대상; 종료 후 보존 산출물(§3 docs 박제 + fixture 5종 + §3-1 미완 잔존 표)은 *외부 도입자 1차 후보 카탈로그*로 영구 보존, 이양 채널 = `/harness:harness-mcp-adopt` + §10-3 프로젝트 유형별 매트릭스 + `/harness:harness-mcp-status`) / **21차 P3-(c) closure ✅** (사용자 외부 PowerShell 인터랙티브 `claude` B8/B9 측정 → B8=B9=6 mode-independent falsification 확정 → 13·15·16차 `-p` 한정 caveat *제거* + §1 [^A] 진짜 절감 채널 후보가 server-side `--toolsets`/env로 유일하게 좁혀짐 — P3 결정적 미해소 분기 완전 closure) / **22차 T1 playwright probe-only ✅** (사용자 외부 PowerShell `node fixtures/probe_playwright.js` 2회 → default COUNT=23 ✓ + `--caps=vision` COUNT=29 +6종 `browser_mouse_*_xy` 좌표 기반 mouse — 17차 docs 박제 정정 2건: network 2종이 *default 포함* / vision 5→6종은 *coordinate-based mouse* (screenshot은 default 이미 포함) + tools/list 단계엔 Chromium 다운로드 미발생 lazy 가설 ✓; §8-3 안전 룰 list 정합 정리 filesystem/time/memory/playwright 4항목 제거) / **23차 T1 chrome-devtools closure ✅ (출처 verify + 🚫 Node engines 함정 박제)** (dharness 본 세션 read-only `WebFetch github.com/ChromeDevTools/chrome-devtools-mcp` + `npm view chrome-devtools-mcp --json` 3-source 교차 = npm Author **Google LLC** + Maintainers `mathias`/`orkon`/`google-wombot`(Google 공식 봇) + Apache-2.0 + mcpName `io.github.ChromeDevTools/chrome-devtools-mcp` + GitHub 39.2k★ + 17차 docs 44 카운트 100% 일치 → 18차 fixture "후보 1" ✅ 승급, spoofing 위험 0; **17차 attach 모드 정정 1건**: auto-launch가 default + remote는 `--browser-url`/`--ws-endpoint` 명시 필요 + `--autoConnect`는 Chrome 144+; **🚫 Node engines 함정 empirical**: 사용자 외부 PowerShell 1회 시도 결과 dharness 본 세션 환경(Node v18.15.0 단일)에서 spawn 차단 `ERROR: chrome-devtools-mcp does not support Node v18.15.0` — npm view 7 버전 cross-check로 모든 49 versions이 `node ^20.19 \|\| ^22.12 \|\| >=23` 요구 확정, **playwright(22차 v18 통과)와 다른 함정 surface**(Node 20.19+ hard requirement 추가 차단 layer); 사용자 결정으로 dharness root에서 시스템 Node 업그레이드 회피 → *명안 박제* closure, derived 프로젝트별 §10 진입 시점에 winget/nvm-windows/portable zip 중 선택 후 외부 PowerShell 1회 spawn → 44종 vs 실 enum diff 검증) / **24차 self-verify ✅** (가상 *프론트엔드/E2E 테스트* derived 프로젝트의 `/harness:harness-mcp-adopt "chrome-devtools로 lighthouse 성능 분석"` 진입 시뮬레이션 — read-only, no spawn — 23차 산출물 4건이 §10 5-step 워크플로우에서 의도대로 발화: Step 1 §3 ✅ 출처 박제 trust source 즉시 인용 / Step 2 §8-3 룰 매칭 drift 0 (출처 trust ✓ vs enum 미완 ✗ 자연 분리) / Step 2 fixture 헤더 🚫 Node engines 함정 → `node --version` 선행 gate 발화 / Step 3 §3-1 잔존 표 "실 enum *영구 외부 의제*" + §3 "📜 docs 박제 44종" caveat 동반 → docs 추정 vs empirical 확정 분리 명확화 → 자동 채택 차단. 15차 `harness-mcp-status` self-verify가 14차 산출물 4건 작동 empirical 검증한 패턴 답습). 결과는 `fixtures/README.md` 결과 로그에 누적.
>
> **운영 함의 (cycle 11 누적):**
