...
>
> **측정 회로 (2026-05-15):** 정적 token cost는 `py plugins/harness/scripts/poc/measure_dogfood.py [--strip-poc-note]` 결정적 측정 — `_workspace/_telemetry/poc_dogfood_static.jsonl` 누적. 실 세션 runtime 측정 protocol + 결정 임계는 `references/poc-dogfood-protocol.md` 박제 (옵션 B-a baseline / B-b swap / B-c 폐기 + ratio < 0.60 + 품질 ≥ 4.0 채택 게이트).
>
> **결정 (2026-05-16, doctrine only):** 옵션 **B-b 채택** — 정적 token ratio 0.5037 (agent-design-patterns 단일) / 0.5763 (4-file aggregate) < 0.60, runtime quality B-a=4 / B-b=4 ≥ 4.0 양조건 충족. 실 swap 운영 적용은 별도 cycle (`test_ko_has_poc_box_with_en_link` + `test_structural_parity_section_count` 2 invariant 재정의 동반). 본 박스 cross-link 표 (L474-479)는 `.md` 유지 — silent swap 금지 doctrine 유효.
...
- **Uplift Baseline Protocol**: `references/uplift-protocol.md` — Q2 no-harness vs harness 세션 정량 비교 measurement (`scripts/uplift/measure_session.py`) + 결정 게이트. **결정 (2026-05-16, 옵션 A doctrine):** 단발 페어 측정 = 모집단 1 + dharness 외부 sandbox fixture 부재 + `tool_output_captured` 10KB threshold 미달 task noise → 결정 게이트 즉시 적용 보류. scaffold (`measure_session.py` + `uplift_sessions.jsonl`) 유지 — 미래 외부 sandbox 다중 fixture 누적 시 게이트 적용.
- **POC Dogfood Protocol**: `references/poc-dogfood-protocol.md` — P7-2 EN POC runtime 측정 protocol (`scripts/poc/measure_dogfood.py` 정적 측정 + 옵션 B-a/b/c)
