...
_DOCTRINE_FN_REF_PATTERN = re.compile(
    r"`(chain|chain_intent|chain_version|chain_doc_sync|structure|schema)\.py:([A-Za-z_][A-Za-z0-9_]*)`"
)
...
_INVERSE_MODULE_ROW_PATTERN = re.compile(
    r"^\|\s*`(chain|chain_intent|chain_version|chain_doc_sync|structure|schema)\.py`[^|]*\|\s*([^|]+)\|",
    re.MULTILINE,
)
