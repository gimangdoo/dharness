...
- 에이전트/스킬 추가만 원함 → `/harness:harness-evolve "X 추가"`(내부 add-agent/add-skill op) 안내 후 중단
- 신규 기능 의도만 추가 원함 → `/harness:harness-feature <기능>` 안내 후 중단
- baseline 갱신만 원함 → `/harness:harness-evolve "baseline 갱신"`(baseline op) 안내 후 중단
...
`plugins/harness/skills/harness/SKILL.md` §산출물 체크리스트의 18개 항목(must 9 / should 9, 단일 출처 `references/output-checklist.md`)을 확인한다. 특히 다음 4개는 Phase 10 작동 및 후속 명령 append 전제조건이므로 반드시 확인:
...
- [ ] `_workspace/_baseline/changelog.md` skeleton 박제됨 (Phase 7-4.1, 후속 `/harness:harness-evolve`·`harness-feature` append 대상)

## 후속 명령어 (7 command)

- 신규 기능 의도 흡수 (profile 확장): `/harness:harness-feature <기능 한 문장>`
- 모든 구조적 변경 (통합 front door — 에이전트/스킬 추가·제거·분할·통합·baseline·MCP 채택 내부 op): `/harness:harness-evolve <피드백>`
- 진행 가시성·진단 단일 진입점 (read-only; `--deep` LLM 감사 / `--mcp` MCP 매트릭스·추천): `/harness:harness-status [--verbose] [--deep] [--mcp]`
- 결정적 일관성 검증 (LLM 0): `/harness:harness-validate [--json] [--strict]`
- drift 점검 (Phase 10 수동 트리거): `/harness:harness-adapt`
- 추궁 모드 (모호 답안 1q씩 확정): `/harness:harness-grill <step> [필드]`
